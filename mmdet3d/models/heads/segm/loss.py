...
class SimpleLoss(torch.nn.Module):
    ...
    def forward(self, ypred, ytgt):
        loss = self.loss_fn(ypred, ytgt.float())
        return loss
    ...
    def forward(self, embedding, seg_gt):
        ...
        for b in range(bs):
            ...
            for lane_idx in labels:
                ...
                # ---------- var_loss -------------
                var_loss = var_loss + \
                    torch.mean(F.relu(torch.norm(embedding_i - mean_i.reshape(self.embed_dim, 1),
            ...
            if num_lanes > 1:
                ...
                dist = dist + torch.eye(num_lanes, dtype=dist.dtype, device=dist.device) * \
