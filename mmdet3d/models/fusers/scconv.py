...
class SCNET(nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        self.enc = nn.Conv2d(in_channels[0], in_channels[1], 3, padding=1, bias=False)
        self.layers = self.make_layer(SCBottleneck, in_channels[1], num_blocks=5)

    @autocast(enabled=False)
    def forward(self, x):
        return self.layers(self.enc(x))
